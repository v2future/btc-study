...
public class Script {
     ...
     * @throws BtcException
     */
    public Script(byte[] programBytes, int offset, int length) throws BtcException {
    //    this.params = params;
    ...
    public String toString() {
        ...
        for (ScriptChunk chunk : chunks) {
            if (chunk.isOpCode) {
                ...
                // Data chunk
                buf.append("[");
                buf.append(BtcKit.toHexString(chunk.data));
                buf.append("] ");
            }
        }
    ...
    private byte[] getData(int len) throws BtcException {
        if (len > program.length - cursor)
            throw new BtcException("Failed read of " + len + " bytes");
        try {
            ...
            throw new BtcException("Failed read of " + len + " bytes", e);
    ...
    private int readByte() throws BtcException {
        try {
            return 0xFF & program[cursor++];
        } catch (ArrayIndexOutOfBoundsException e) {
            throw new BtcException("Attempted to read outside of script boundaries");
        }
    }
     ...
     */
    private void parse(byte[] programBytes, int offset, int length) throws BtcException {
     ...
     * then this function retrieves the third element, otherwise it throws a BtcException.<p>
     *
     * This is useful for fetching the destination address of a transaction.
     */
    public byte[] getPubKeyHash() throws BtcException {
        if (!isSentToAddress())
            throw new BtcException("Script not in the standard scriptPubKey form");
     ...
     *
     * @throws BtcException if the script is none of the named forms.
     */
    public byte[] getPubKey() throws BtcException {
        if (chunks.size() != 2) {
            throw new BtcException("Script not of right size, expecting 2 but got " + chunks.size());
        }
        if (chunks.get(0).data.length > 2 && chunks.get(1).data.length > 2) {
            ...
            throw new BtcException("Script did not match expected form: " + toString());
        }
    }
     ...
     */
    public Address getFromAddress() throws BtcException {
        return new Address( BtcKit.sha256hash160(getPubKey()));
    }
     ...
     * @throws BtcException
     */
    public Address getToAddress() throws BtcException {
    ...
    private static int getSigOpCount(List<ScriptChunk> chunks, boolean accurate) throws BtcException {
        int sigOps = 0;
     ...
     */
    private static int getOpNValue(int opcode) throws BtcException {
        if (opcode == OP_0)
            return 0;
        if (opcode < OP_1 || opcode > OP_16) // This should absolutely never happen
            throw new BtcException("getOpNValue called on non OP_N opcode");
     ...
     */
    public static int getSigOpCount(byte[] program) throws BtcException {
     ...
     */
    public static long getP2SHSigOpCount(byte[] scriptSig) throws BtcException {
        Script script = new Script();
        try {
    ...
    private static BigInteger castToBigInteger(byte[] chunk) throws BtcException {
        if (chunk.length > 4)
            throw new BtcException("Script attempted to use an integer larger than 4 bytes");
        return Utils.decodeMPI(Utils.reverseBytes(chunk), false);
    }
    
    private static void executeScript(Transaction txContainingThis, long index,
                                      Script script, LinkedList<byte[]> stack) throws BtcException {
        int opCount = 0;
        ...
        for (ScriptChunk chunk : script.chunks) {
            ...
            if (!chunk.isOpCode) {
                if (chunk.data.length > 520)
                    throw new BtcException("Attempted to push a data string larger than 520 bytes");
                ...
                if (opcode > OP_16) {
                    opCount++;
                    if (opCount > 201)
                        throw new BtcException("More script operations than is allowed");
                }
                
                if (opcode == OP_VERIF || opcode == OP_VERNOTIF)
                    throw new BtcException("Script included OP_VERIF or OP_VERNOTIF");
                
                if (opcode == OP_CAT || opcode == OP_SUBSTR || opcode == OP_LEFT || opcode == OP_RIGHT ||
                    opcode == OP_INVERT || opcode == OP_AND || opcode == OP_OR || opcode == OP_XOR ||
                    opcode == OP_2MUL || opcode == OP_2DIV || opcode == OP_MUL || opcode == OP_DIV ||
                    opcode == OP_MOD || opcode == OP_LSHIFT || opcode == OP_RSHIFT)
                    throw new BtcException("Script included a disabled Script Op.");
                ...
                case OP_IF:
                    ...
                    if (stack.size() < 1)
                        throw new BtcException("Attempted OP_IF on an empty stack");
                ...
                case OP_NOTIF:
                    ...
                    if (stack.size() < 1)
                        throw new BtcException("Attempted OP_NOTIF on an empty stack");
                    ifStack.add(!castToBool(stack.pollLast()));
                    continue;
                case OP_ELSE:
                    if (ifStack.isEmpty())
                        throw new BtcException("Attempted OP_ELSE without OP_IF/NOTIF");
                    ifStack.add(!ifStack.pollLast());
                    continue;
                case OP_ENDIF:
                    if (ifStack.isEmpty())
                        throw new BtcException("Attempted OP_ENDIF without OP_IF/NOTIF");
                    ifStack.pollLast();
                    continue;
                }
                ...
                case OP_VERIFY:
                    if (stack.size() < 1)
                        throw new BtcException("Attempted OP_VERIFY on an empty stack");
                    if (!castToBool(stack.pollLast()))
                        throw new BtcException("OP_VERIFY failed");
                    break;
                case OP_RETURN:
                    throw new BtcException("Script called OP_RETURN");
                case OP_TOALTSTACK:
                    if (stack.size() < 1)
                        throw new BtcException("Attempted OP_TOALTSTACK on an empty stack");
                    altstack.add(stack.pollLast());
                    break;
                case OP_FROMALTSTACK:
                    if (altstack.size() < 1)
                        throw new BtcException("Attempted OP_TOALTSTACK on an empty altstack");
                    stack.add(altstack.pollLast());
                    break;
                case OP_2DROP:
                    if (stack.size() < 2)
                        throw new BtcException("Attempted OP_2DROP on a stack with size < 2");
                    stack.pollLast();
                    stack.pollLast();
                    break;
                case OP_2DUP:
                    if (stack.size() < 2)
                        throw new BtcException("Attempted OP_2DUP on a stack with size < 2");
                ...
                case OP_3DUP:
                    if (stack.size() < 3)
                        throw new BtcException("Attempted OP_3DUP on a stack with size < 3");
                ...
                case OP_2OVER:
                    if (stack.size() < 4)
                        throw new BtcException("Attempted OP_2OVER on a stack with size < 4");
                ...
                case OP_2ROT:
                    if (stack.size() < 6)
                        throw new BtcException("Attempted OP_2ROT on a stack with size < 6");
                ...
                case OP_2SWAP:
                    if (stack.size() < 4)
                        throw new BtcException("Attempted OP_2SWAP on a stack with size < 4");
                ...
                case OP_IFDUP:
                    if (stack.size() < 1)
                        throw new BtcException("Attempted OP_IFDUP on an empty stack");
                ...
                case OP_DROP:
                    if (stack.size() < 1)
                        throw new BtcException("Attempted OP_DROP on an empty stack");
                    stack.pollLast();
                    break;
                case OP_DUP:
                    if (stack.size() < 1)
                        throw new BtcException("Attempted OP_DUP on an empty stack");
                    stack.add(stack.getLast());
                    break;
                case OP_NIP:
                    if (stack.size() < 2)
                        throw new BtcException("Attempted OP_NIP on a stack with size < 2");
                ...
                case OP_OVER:
                    if (stack.size() < 2)
                        throw new BtcException("Attempted OP_OVER on a stack with size < 2");
                ...
                case OP_ROLL:
                    if (stack.size() < 1)
                        throw new BtcException("Attempted OP_PICK/OP_ROLL on an empty stack");
                    long val = castToBigInteger(stack.pollLast()).longValue();
                    if (val < 0 || val >= stack.size())
                        throw new BtcException("OP_PICK/OP_ROLL attempted to get data deeper than stack size");
                ...
                case OP_ROT:
                    if (stack.size() < 3)
                        throw new BtcException("Attempted OP_ROT on a stack with size < 3");
                ...
                case OP_TUCK:
                    if (stack.size() < 2)
                        throw new BtcException("Attempted OP_SWAP on a stack with size < 2");
                ...
                case OP_LEFT:
                case OP_RIGHT:
                    throw new BtcException("Attempted to use disabled Script Op.");
                case OP_SIZE:
                    if (stack.size() < 1)
                        throw new BtcException("Attempted OP_SIZE on an empty stack");
                ...
                case OP_EQUAL:
                    if (stack.size() < 2)
                        throw new BtcException("Attempted OP_EQUALVERIFY on a stack with size < 2");
                ...
                case OP_EQUALVERIFY:
                    ...
                    if (!Arrays.equals(stack.pollLast(), stack.pollLast()))
                        throw new BtcException("OP_EQUALVERIFY: non-equal data");
                    break;
                case OP_1ADD:
                case OP_1SUB:
                case OP_NEGATE:
                ...
                case OP_0NOTEQUAL:
                    if (stack.size() < 1)
                        throw new BtcException("Attempted a numeric op on an empty stack");
                ...
                case OP_MAX:
                    if (stack.size() < 2)
                        throw new BtcException("Attempted a numeric op on a stack with size < 2");
                ...
                case OP_NUMEQUALVERIFY:
                    if (stack.size() < 2)
                        throw new BtcException("Attempted OP_NUMEQUALVERIFY on a stack with size < 2");
                    ...
                    if (!OPNUMEQUALVERIFYnum1.equals(OPNUMEQUALVERIFYnum2))
                        throw new BtcException("OP_NUMEQUALVERIFY failed");
                    break;
                case OP_WITHIN:
                    if (stack.size() < 3)
                        throw new BtcException("Attempted OP_WITHIN on a stack with size < 3");
                ...
                case OP_RIPEMD160:
                    if (stack.size() < 1)
                        throw new BtcException("Attempted OP_RIPEMD160 on an empty stack");
                ...
                case OP_SHA1:
                    if (stack.size() < 1)
                        throw new BtcException("Attempted OP_SHA1 on an empty stack");
                    try {
                ...
                case OP_SHA256:
                    if (stack.size() < 1)
                        throw new BtcException("Attempted OP_SHA256 on an empty stack");
                    try {
                ...
                case OP_HASH160:
                    if (stack.size() < 1)
                        throw new BtcException("Attempted OP_HASH160 on an empty stack");
                ...
                case OP_CHECKSIGVERIFY:
                    if (stack.size() < 2)
                        throw new BtcException("Attempted OP_CHECKSIG(VERIFY) on a stack with size < 2");
                    ...
                    byte[] sigHashByte = txContainingThis.hashTransactionForSignature((int)index, CHECKSIGconnectedScript,
                            CHECKSIGsig[CHECKSIGsig.length - 1]);
                                        
                    boolean CHECKSIGsigValid;
                    try {
                        CHECKSIGsigValid = ECKey.verify(sigHashByte, Arrays.copyOf(CHECKSIGsig, CHECKSIGsig.length - 1), CHECKSIGpubKey);
                    } catch (Exception e1) {
                    ...
                    if (opcode == OP_CHECKSIG)
                        ...
                        if (!CHECKSIGsigValid)
                            throw new BtcException("Script failed OP_CHECKSIGVERIFY");
                    break;
                case OP_CHECKMULTISIG:
                case OP_CHECKMULTISIGVERIFY:
                    if (stack.size() < 2)
                        throw new BtcException("Attempted OP_CHECKMULTISIG(VERIFY) on a stack with size < 2");
                    ...
                    if (CHECKMULTISIGpubKeyCount < 0 || CHECKMULTISIGpubKeyCount > 20)
                        throw new BtcException("OP_CHECKMULTISIG(VERIFY) with pubkey count out of range");
                    opCount += CHECKMULTISIGpubKeyCount;
                    if (opCount > 201)
                        throw new BtcException("Total op count > 201 during OP_CHECKMULTISIG(VERIFY)");
                    if (stack.size() < CHECKMULTISIGpubKeyCount + 1)
                        throw new BtcException("Attempted OP_CHECKMULTISIG(VERIFY) on a stack with size < num_of_pubkeys + 2");
                    ...
                    if (CHECKMULTISIGsigCount < 0 || CHECKMULTISIGsigCount > CHECKMULTISIGpubKeyCount)
                        throw new BtcException("OP_CHECKMULTISIG(VERIFY) with sig count out of range");
                    if (stack.size() < CHECKMULTISIGsigCount + 1)
                        throw new BtcException("Attempted OP_CHECKMULTISIG(VERIFY) on a stack with size < num_of_pubkeys + num_of_signatures + 3");
                    ...
                    while (CHECKMULTISIGsigs.size() > 0) {
                        ...
                        byte[] CHECKMULTISIGhash = txContainingThis.hashTransactionForSignature((int)index, CHECKMULTISIGconnectedScript,
                                CHECKMULTISIGsig[CHECKMULTISIGsig.length - 1]);
                        try {
                            if (ECKey.verify(CHECKMULTISIGhash, Arrays.copyOf(CHECKMULTISIGsig, CHECKMULTISIGsig.length - 1), CHECKMULTISIGpubKey))
                    ...
                    if (opcode == OP_CHECKMULTISIG)
                        ...
                        if (!CHECKMULTISIGValid)
                            throw new BtcException("Script failed OP_CHECKMULTISIGVERIFY");
                    break;
                case OP_NOP1:
                case OP_NOP2:
                case OP_NOP3:
                ...
                default:
                    throw new BtcException("Script used a reserved Op Code");
                }
            }
            
            if (stack.size() + altstack.size() > 1000 || stack.size() + altstack.size() < 0)
                throw new BtcException("Stack size exceeded range");
        }
        
        if (!ifStack.isEmpty())
            throw new BtcException("OP_IF/OP_NOTIF without OP_ENDIF");
    }
    ...
    public void correctlySpends(Transaction txContainingThis, long scriptSigIndex, Script scriptPubKey,
                                boolean enforceP2SH) throws BtcException {
        if (program.length > 10000 || scriptPubKey.program.length > 10000)
            throw new BtcException("Script larger than 10,000 bytes");
        ...
        if (stack.size() == 0)
            throw new BtcException("Stack empty at end of script execution.");
        
        if (!castToBool(stack.pollLast()))
            throw new BtcException("Script resulted in a non-true stack");
        ...
        if (enforceP2SH && scriptPubKey.isPayToScriptHash()) {
            for (ScriptChunk chunk : chunks)
                if (chunk.isOpCode && (chunk.data[0] & 0xff) > OP_16)
                    throw new BtcException("Attempted to spend a P2SH scriptPubKey with a script that contained script ops");
            ...
            if (p2shStack.size() == 0)
                throw new BtcException("P2SH stack empty at end of script execution.");
            
            if (!castToBool(p2shStack.pollLast()))
                throw new BtcException("P2SH script execution resulted in a non-true stack");
        }
    }
}

...
public class Base58 {
    ...
    public static byte[] decode(String input) throws BtcException {
        if (input.length() == 0) {
            return new byte[0];
        }
        ...
        for (int i = 0; i < input.length(); ++i) {
            ...
            if (digit58 < 0) {
                throw new BtcException("Illegal character " + c + " at " + i);
            }
    ...
    public static BigInteger decodeToBigInteger(String input) throws BtcException {
     ...
     *
     * @throws BtcException if the input is not base 58 or the checksum does not validate.
     */
    /*
    public static byte[] decodeChecked(String input) throws BtcException {
        byte tmp [] = decode(input);
        if (tmp.length < 4)
            throw new BtcException("Input to short");
        byte[] bytes = copyOfRange(tmp, 0, tmp.length - 4);
        byte[] checksum = copyOfRange(tmp, tmp.length - 4, tmp.length);
        tmp = Utils.doubleDigest(bytes);
        byte[] hash = copyOfRange(tmp, 0, 4);
        if (!Arrays.equals(checksum, hash)) 
            throw new BtcException("Checksum does not validate");
